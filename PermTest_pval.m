function [signode,GLPstat,P_value,Tmax_5prct,max_T0]=PermTest_pval(Wt,Wt2,Cindx,CID,thresh,M)
...
p_value=1-diff_ind/size(T_max,1);
...
Cinx = find(T_k0>0);
for j=1:size(Cinx,1)
    [diff_value,diff_ind]=min(abs(sort(T_max)-T_k0(Cinx(j))));
    P_value(j)=1-diff_ind/size(T_max,1);
end
